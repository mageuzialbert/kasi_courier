...
export async function PUT(
  ...
  try {
    ...
    const { allowed, error: permError } = await requirePermission(user.id, role || '', 'deliveries.assign');
    if (!allowed) {
      return NextResponse.json(
    ...
    if (delivery.status !== 'PENDING_CONFIRMATION') {
      return NextResponse.json(
        { error: `Cannot confirm delivery with status: ${delivery.status}. Only PENDING_CONFIRMATION deliveries can be confirmed.` },
        { status: 400 }
      );
    }
